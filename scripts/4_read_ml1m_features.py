...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the user data file (e.g., users.dat)",
    )
    parser.add_argument(
        "--item-data-path",
        type=str,
        required=True,
        help="Path to the item data file (e.g., movies.dat)",
    )
    parser.add_argument(
        "--user-encoder-path",
        type=str,
...
def process_user_data(
    ...
    """Reads users.dat, encodes features, and saves users.csv + encoders."""
...
def process_item_data(
    ...
    """Reads movies.dat, encodes features, and saves items.csv + encoders."""
    ...
    genres_onehot.columns = [f"genre_{g}" for g in genres_onehot.columns]

    # 5. Combine
    final_df = pd.concat([df[["item_id", "norm_year"]], genres_onehot], axis=1)
    ...
    for col in genres_onehot.columns:
...
def main():
    """Main function to process ML1M user and item features."""
    # 1. Parse Args
    (
        user_data_path,
        item_data_path,
