...
def make_not_hook_factory(get_event_embeddings, NOT, sim, reg_weight: float):
    """
    Creates a hook factory for the NOT module.
    Produces: hook(param) → function to attach with param.register_hook
    """

    def factory(param):
        def hook(grad):
            X = get_event_embeddings()
            if X is None:
                return grad

            R = not_regularizer(NOT, X, sim=sim)

            reg_grad = torch.autograd.grad(
                R, param, retain_graph=True, allow_unused=True
            )[0]

            if reg_grad is None:
                return grad

            return grad + reg_weight * reg_grad
...
def make_and_hook_factory(get_event_embeddings, TRUE, AND, NOT, sim, reg_weight: float):
    """
    Creates a hook factory for the AND module.
    Produces: hook(param) → function to attach with param.register_hook
    """
    ...
    def factory(param):
        def hook(grad):
            ...
            R = and_regularizer(AND, NOT, TRUE, X, sim=sim)
...
def make_or_hook_factory(get_event_embeddings, TRUE, OR, NOT, sim, reg_weight: float):
    """
    Creates a hook factory for the OR module.
    Produces: hook(param) → function to attach with param.register_hook
    """
    ...
    def factory(param):
        def hook(grad):
            ...
            R = or_regularizer(OR, NOT, TRUE, X, sim=sim)
